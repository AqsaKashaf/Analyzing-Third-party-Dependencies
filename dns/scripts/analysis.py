...
def plotWebsiteStats(stats,total):
    ...
    r5 = ax.bar(x+width*2.5, third_pvt, width, label="3rd + Pvt",hatch=patterns[8],color='white', edgecolor='black')
    ...
    lgd = ax.legend(bbox_to_anchor=(0.5,1.37),loc="upper center",ncol=2)
    autolabel(r1,ax)
    autolabel(r2,ax)
    autolabel(r3,ax)
    autolabel(r4,ax)
...
def main():
    ...
    #             grpid = groupMAP[p]
    #             # print(f"{r},{w},{grpid}")
    #             fin_providers.add(grpid)
    #         except KeyError:
    #             # print(f"{r},{w},{p}")
    #             fin_providers.add(p)
        
    #     for p in fin_providers:
    #         print(f"{r},{w},{p}")
